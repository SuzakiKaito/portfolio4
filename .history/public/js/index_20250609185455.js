...
// skillSection が display: block になったタイミングでクラス追加
function checkSkillVisibility() {
  const isVisible = getComputedStyle(skillSection).display !== 'none';
  if (isVisible) {
    workContent.classList.add('skill-active');
  } else {
    workContent.classList.remove('skill-active');
  }
}
...
// 必要に応じて表示切り替え時にも実行
// 例: ボタンクリックで skillSection の表示を切り替える場合
document.querySelector('.skillMenu a').addEventListener('click', () => {
